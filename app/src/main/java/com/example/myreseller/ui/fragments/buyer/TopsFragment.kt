...
class TopsFragment : Fragment() {
    ...
    private fun fetchData(){
        ...
        FirebaseFirestore.getInstance().collection(Constants.ITEMS).whereEqualTo("category","Tops").get()
            .addOnSuccessListener { documents ->
                for (document in documents){
                    //model and adapter
                    val item = documents.toObjects(Item::class.java)
                    binding.rvTopsBuyer.adapter = AllProductsBuyerRecyclerAdapter(requireContext(),item)
                }
