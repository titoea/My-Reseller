...
import com.example.myreseller.databinding.FragmentDressesBinding
 ...
 * Use the [DressesFragment.newInstance] factory method to
...
class DressesFragment : Fragment() {
    ...
    private fun fetchData(){
        ...
        FirebaseFirestore.getInstance().collection(Constants.ITEMS).whereEqualTo("category","Dresses").get()
            .addOnSuccessListener { documents ->
                for (document in documents){
                    //model and adapter
                    val item = documents.toObjects(Item::class.java)
                    binding.rvDressesBuyer.adapter = AllProductsBuyerRecyclerAdapter(requireContext(),item)
                }
