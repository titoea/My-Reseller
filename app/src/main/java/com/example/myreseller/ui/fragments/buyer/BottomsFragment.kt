...
import com.example.myreseller.databinding.FragmentBottomsBinding
 ...
 * Use the [BottomsFragment.newInstance] factory method to
 * create an instance of this fragment.
 */
class BottomsFragment : Fragment() {
    private var _binding: FragmentBottomsBinding? = null
    ...
    override fun onCreateView(
        ...
        // Inflate the layout for this fragment
        _binding = FragmentBottomsBinding.inflate(inflater, container, false)

        val root: View = binding.root

        binding.rvBottomsBuyer.apply {
    ...
    private fun fetchData(){
        ...
        FirebaseFirestore.getInstance().collection(Constants.ITEMS).whereEqualTo("category","Bottoms").get()
            .addOnSuccessListener { documents ->
                for (document in documents){
                    //model and adapter
                    val item = documents.toObjects(Item::class.java)
                    binding.rvBottomsBuyer.adapter = AllProductsBuyerRecyclerAdapter(requireContext(),item)
                }
